...
- **ללא תמונות של פנים או אנשים מזוהים** בשום חלק באתר (אווטרים, פורטרטים, רקעים). מותרת תמונה צנועה של ידיים / לחיצת יד **ללא פנים** (כמו ב-Hero בעמוד הבית — `frontend/public/hero-handshake.jpg`). בפרסומות הכלל מחמיר יותר: אסורות תמונות אנשים כלל (ראו כלל נפרד למטה).
...
- **לשון זכר בלבד בכל האתר.** אין ניסוח כפול מגדרי (לא "מועמד/ת", לא "תמצא/י", לא "מוכן/ה"). כל פנייה למשתמש, תוויות, placeholders ודוגמאות — בלשון זכר.
